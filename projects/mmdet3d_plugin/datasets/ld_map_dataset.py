...
class LiDARInstanceLines(object):
    ...
    def fixed_num_sampled_points(self):
        ...
        for instance in self.instance_list:
            distances = np.linspace(0, instance.length, self.fixed_num)  # The purpose of this code is to create a fixed number of evenly spaced distances along the length of the map instance.
            sampled_points = np.array([list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, 2)  # uses the distances array to sample points along the map instance using the instance's interpolate method
...
class CustomLDLocalMapDataset(CustomNuScenesDataset):
    r"""LD Dataset.
