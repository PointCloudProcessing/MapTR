...
optimizer = dict(
    type='AdamW',
    lr=1e-4,  # This sets the initial learning rate for the optimizer to 6e-4. The learning rate determines the step size during gradient descent
    paramwise_cfg=dict(
        custom_keys={
            'img_backbone': dict(lr_mult=0.02), # the learning rate is multiplied by 0.1, meaning it will be 10 times smaller than the base learning rate (6e-4)
        }),
    weight_decay=0.02,
    eps = 1e-8) # controls the L2 regularization strength (weight decay). It applies a penalty to the model's weights to discourage them from becoming too large
...
optimizer_config = dict(grad_clip= # which is a technique used to prevent gradients from becoming too large and potentially causing numerical instability 
                        dict(max_norm=50,  # This sets the maximum L2 norm of gradients to 35. If any gradient's L2 norm exceeds this value, it will be scaled down to ensure it does not exceed this threshold.
                                       norm_type=2)) # This specifies the type of norm
# learning policy
lr_config = dict(
    policy='CosineAnnealing',
    warmup='linear',
    warmup_iters=100, #  specifies the number of initial iterations during which the learning rate gradually increases from its initial value to the regular learning rate.
    warmup_ratio=1.0 / 6, # specifies the ratio of the initial learning rate to the regular learning rate. A larger warmup_ratio means a longer warm-up phase, during which the learning rate increases gradually. This can be useful to ensure the model starts with a very low learning rate and spends a significant portion of training with smaller steps.
