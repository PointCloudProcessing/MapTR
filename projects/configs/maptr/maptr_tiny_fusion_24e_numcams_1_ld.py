...
dataset_type = 'CustomLDLocalMapDataset'

data_root = '/media/NAS/raw_data/ShuoShen/VectorMapNet_data/0_Build_Dataset'
...
test_pipeline = [
    dict(type='CustomLoadLDImageFromBags', to_float32=True),
    dict(type='NormalizeMultiviewImage', **img_norm_cfg),
    dict(type='CustomLoadLDPointsFromFile', coord_type='LIDAR', load_dim=load_dim, use_dim=use_dim, reduce_beams=reduce_beams),
    dict(type='CustomLoadLDPointsFromMultiSweeps', sweeps_num=9, load_dim=load_dim, use_dim=use_dim, reduce_beams=reduce_beams, pad_empty_sweeps=True, remove_close=True),
    dict(type='CustomPointsRangeFilter', point_cloud_range=lidar_point_cloud_range),
    dict(
...
ld_pipeline = [
    ...
    dict(
        ...
        patch_size=(point_cloud_range[5]-point_cloud_range[1],point_cloud_range[4]-point_cloud_range[0]),
...
optimizer = dict(
    type='AdamW',
    lr=1e-5,  # This sets the initial learning rate for the optimizer to 6e-4. The learning rate determines the step size during gradient descent
    paramwise_cfg=dict(
        custom_keys={
...
optimizer_config = dict(grad_clip= # which is a technique used to prevent gradients from becoming too large and potentially causing numerical instability 
                        dict(max_norm=25,  # This sets the maximum L2 norm of gradients to 35. If any gradient's L2 norm exceeds this value, it will be scaled down to ensure it does not exceed this threshold.
                                       norm_type=2)) # This specifies the type of norm
# learning policy
lr_config = dict(
    policy='CosineAnnealing',
    warmup='linear',
    warmup_iters=10, #  specifies the number of initial iterations during which the learning rate gradually increases from its initial value to the regular learning rate.
    warmup_ratio=1.0 / 4, # specifies the ratio of the initial learning rate to the regular learning rate. A larger warmup_ratio means a longer warm-up phase, during which the learning rate increases gradually. This can be useful to ensure the model starts with a very low learning rate and spends a significant portion of training with smaller steps.
